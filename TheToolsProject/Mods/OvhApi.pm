...
sub rawCall
{
    ...
    if (not $params{'path'})
    {
        ...
        return Mods::OvhAnswer::->new(response => HTTP::Response->new( 500, "Missing parameter: path", [], '{"message":"Missing parameter: path"}'));
    }
    if (not $params{'method'})
    {
        carp "Missing parameter: method";
        return Mods::OvhAnswer::->new(response => HTTP::Response->new( 500, "Missing parameter: method", [], '{"message":"Missing parameter: method"}'));
    }
    ...
    unless ($params{'noSignature'})
    {
        ...
        if (not $self->{'consumerKey'})
        {
            ...
            return Mods::OvhAnswer::->new(response => HTTP::Response->new( 500, "Performed an authentified call without providing a valid consumerKey", [], '{"message":"Performed an authentified call without providing a valid consumerKey"}'));
        }
    ...
    return Mods::OvhAnswer::->new(response => $UserAgent->$method($url, %httpHeaders, %content));
}
...
Answer are retured as instances of L<Mods::OvhAnswer|Mods::OvhAnswer>.
