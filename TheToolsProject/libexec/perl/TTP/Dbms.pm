...
sub backupDatabase {
	...
	if( !TTP::Toops::ttpErrs()){
		...
		my $res = TTP::Dbms::toPackage( 'apiBackupDatabase', $dbms, $parms );
...
sub execSqlCommand {
	...
	my $result = TTP::Dbms::toPackage( 'apiExecSqlCommand', $dbms, $parms );
	if( $result && $result->{ok} ){
		my $tabular = true;
...
sub getDatabaseTables {
	my ( $database ) = @_;
	my $result = TTP::Dbms::toPackage( 'apiGetDatabaseTables', undef, $database );
...
sub getLiveDatabases {
	my ( $dbms ) = @_;
	my $result = TTP::Dbms::toPackage( 'apiGetInstanceDatabases', $dbms );
	return $result->{output} || [];
}
