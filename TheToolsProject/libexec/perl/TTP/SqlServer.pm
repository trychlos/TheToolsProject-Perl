...
sub apiBackupDatabase {
	my ( $me, $dbms, $parms ) = @_;
	my $result = { ok => false };
	msgErr( "SqlServer::apiBackupDatabase() instance is mandatory, but not specified" ) if !$dbms || !$dbms->{instance} || !$dbms->{instance}{name};
	...
	if( !TTP::Toops::ttpErrs()){
		...
		msgVerbose( "SqlServer::apiBackupDatabase() sql='$parms->{sql}'" );
		$result = _sqlExec( $dbms, $parms->{sql} );
	}
	msgVerbose( "SqlServer::apiBackupDatabase() returns '".( $result->{ok} ? 'true':'false' )."'" );
	return $result;
}
...
# (I):
# - the name of this dynamically addressed module as a string 'TTP::SqlServer'
...
sub apiExecSqlCommand {
	...
	msgVerbose( "SqlServer::apiExecSqlCommand() result='".( $result->{ok} ? 'true' : 'false' )."'" );
	return $result;
}
...
sub apiGetDatabaseTables {
	...
	if( !TTP::Toops::ttpErrs()){
		...
		if( $result->{ok} ){
			...
			msgVerbose( "SqlServer::apiGetDatabaseTables() found ".scalar @{$result->{output}}." tables" );
		}
	}
	msgVerbose( "SqlServer::apiGetDatabaseTables() result='".( $result->{ok} ? 'true' : 'false' )."'" );
	return $result;
}
...
sub apiGetInstanceDatabases {
	...
	if( !TTP::Toops::ttpErrs()){
		...
		if( $result->{ok} ){
			...
			msgVerbose( "SqlServer::apiGetInstanceDatabases() found ".scalar @{$result->{output}}." databases" );
		}
	}
	msgVerbose( "SqlServer::apiGetInstanceDatabases() result='".( $result->{ok} ? 'true' : 'false' )."'" );
	return $result;
}
...
sub apiRestoreDatabase {
	...
	msgVerbose( "SqlServer::apiRestoreDatabase() result='".( $result->{ok} ? 'true' : 'false' )."'" );
	return $result;
}
...
sub _connect {
	...
	if( $sqlsrv ){
		msgVerbose( "SqlServer::_connect() instance already connected" );
	} else {
		my $instance = $dbms->{instance}{name};
		if( $instance ){
			Win32::SqlServer::SetDefaultForEncryption( 'Optional', true );
			my( $account, $passwd ) = TTP::SqlServer::_getCredentials( $dbms );
			if( length $account && length $passwd ){
				...
				msgVerbose( "SqlServer::_connect() calling sql_init with server='".( $server || '(undef)' )."', account='$account'..." );
				$sqlsrv = Win32::SqlServer::sql_init( $server, $account, $passwd );
				msgVerbose( "SqlServer::_connect() sqlsrv->isconnected()=".$sqlsrv->isconnected());
				#print Dumper( $sqlsrv );
				if( $sqlsrv && $sqlsrv->isconnected()){
					$sqlsrv->{ErrInfo}{MaxSeverity} = 17;
					$sqlsrv->{ErrInfo}{SaveMessages} = 1;
					msgVerbose( "SqlServer::_connect() successfully connected" );
					$dbms->{instance}{sqlsrv} = $sqlsrv;
				} else {
					msgErr( "SqlServer::_connect() unable to connect to '$instance' instance" );
					$sqlsrv = undef;
				}
			} else {
				msgErr( "SqlServer::_connect() unable to get account/password couple" );
			}
		} else {
			msgErr( "SqlServer::_connect() instance is mandatory, not specified" );
		}
	}
	return $sqlsrv;
}
...
sub _databaseExists {
	...
	msgVerbose( "SqlServer::_databaseExists() database='$database' exists='".( $exists ? 'true' : 'false' )."'" );
	return $exists;
}
...
sub _getCredentials {
	my ( $dbms ) = @_;
	my $credentials = TTP::Credentials::get([ 'DBMS', $dbms->{instance}{name}, ]);
	my $account = undef;
	my $passwd = undef;
	if( $credentials ){
		$account = ( keys %{$credentials} )[0];
		$passwd = $credentials->{$account};
		msgVerbose( "SqlServer::_getCredentials() got account='".( $account || '(undef)' )."'" );
	} else {
		msgErr( "SqlServer::_getCredentials() unable to get credentials with provided arguments" );
	}
...
sub _restoreDatabaseFile {
	...
	my $last = $parms->{last};
	#
	msgVerbose( "SqlServer::_restoreDatabaseFile() restoring $fname" );
...
sub _restoreDatabaseMove {
	...
	if( !scalar @{$result->{result}} ){
		msgErr( "SqlServer::_restoreDatabaseMove() unable to get the files list of the backup set" );
	} else {
		my $sqlDataPath = TTP::Path::withTrailingSeparator( $dbms->{config}{DBMSInstances}{$parms->{instance}}{dataPath} );
		foreach( @{$result->{result}} ){
			my $row = $_;
			$move .= ', ' if length $move;
...
sub _restoreDatabaseVerify {
	...
	my $database = $parms->{database};
	my $fname = $parms->{file};
	msgVerbose( "SqlServer::_restoreDatabaseVerify() verifying $fname" );
	my $move = _restoreDatabaseMove( $dbms, $parms );
...
sub _sqlExec {
	...
	my $sqlsrv = undef;
	if( !TTP::Toops::ttpErrs()){
		$sqlsrv = TTP::SqlServer::_connect( $dbms );
	}
	if( !TTP::Toops::ttpErrs()){
	...
	msgVerbose( "SqlServer::_sqlExec() returns '".( $result->{ok} ? 'true':'false' )."'" );
	return $result;
}
