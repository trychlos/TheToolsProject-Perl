	...
	select
		...
		left join ( select A.ModuleStagiaireID, sum( A.NMinutes ) as AbsencesMinutes, count( A.DateHourFrom ) as AbsencesCount
			from dbo.c_StagiairesSeancesIntraVars A where A.DateHourFrom < GETDATE() and A.TypePresence <> 'PRS' group by A.ModuleStagiaireID ) ABSENCES on ABSENCES.ModuleStagiaireID = RES.ModuleStagiaireID
	...
	select
		...
		left join ( select A.CoursID, count( A.DayDate ) as SignFormateurManquantes
			from dbo.c_SeancesInterVars A where A.DayDate < GETDATE() and A.SignImage is null group by A.CoursID ) SIGN_FORMATEUR_MANQUANTES on SIGN_FORMATEUR_MANQUANTES.CoursID = COURS.ID
		...
		left join ( select A.CoursStagiaireID, sum( A.NMinutes ) as AbsencesMinutes, count( A.DateHourFrom ) as AbsencesCount
			from dbo.c_StagiairesSeancesInterVars A where A.DateHourFrom < GETDATE() and A.TypePresence <> 'PRS' group by A.CoursStagiaireID ) ABSENCES on ABSENCES.CoursStagiaireID = RES.CoursStagiaireID
