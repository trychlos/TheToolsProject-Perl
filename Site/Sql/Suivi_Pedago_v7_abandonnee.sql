	...
	-- pwi 2025- 1-20 v7 ignore 'no show' notes pedago
	...
	select
		...
		left join ( select B.ModuleID, COUNT(*) as NotesPedagoFound from dbo.c_SeancesIntraVarPedagos A
			...
			where ( ISNULL( A.Label, '' ) <> '' or ISNULL( A.Remarques, '' ) <> '' or ISNULL( A.Conseils, '' ) <> '' or ISNULL( A.TravailPerso, '' ) <> '' )
				and replace( A.Label, ' ', '' ) not like '%noshow%' and replace( A.Remarques, ' ', '' ) like '%noshow%' and replace( A.Conseils, ' ', '' ) like '%noshow%' and replace( A.TravailPerso, ' ', '' ) like '%noshow%'
	...
	-- where NOTES_PEDAGO_DUE.NotesPedagoDue < NOTES_PEDAGO_FOUND.NotesPedagoFound
