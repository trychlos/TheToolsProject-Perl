	...
	select
		...
		left join ( select A.CoursStagiaireID, sum( A.NMinutes ) as AbsencesMinutes, count( A.DateHourFrom ) as AbsencesCount
			from dbo.c_StagiairesSeancesInterVars A where A.DateHourFrom < GETDATE() and A.DateAbs is not null group by A.CoursStagiaireID ) ABSENCES on ABSENCES.CoursStagiaireID = RES.CoursStagiaireID
